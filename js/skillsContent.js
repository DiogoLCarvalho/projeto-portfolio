const skillCard = [
    {
        nome: 'Adivinhe o número',
        img: 'img/projects01.jpg',
        text: 'Projeto JavaScript, HTML e CSS do jogo "Guess the game". No projeto haverá um número aleatório de 1 a 100 que você terá que adivinhar. A cada tentativa aparecerá uma mensagem indicando se o seu palpite foi muito alto ou baixo.',
        link:'https://diogolcarvalho.github.io/projeto-guess-the-number/',
        alt:'Primeira imagem conceitual representando o projeto Adivinhe o número',
        icons: ['fab fa-js-square fa-3x', 'fab fa-html5 fa-3x','fab fa-css3-alt fa-3x']
    
    },
    {
        nome: 'Primeiro Portfólio',
        img: 'img/projects02.jpg',
        text: 'Este é o meu primeiro portfólio. Desenvolvido por conta própria. Ele é responsivo, aos meus testes. Possui apenas o básico de JS. One page site, como este portfólio, possui algumas semelhanças com este projeto.',
        link:'https://diogolcarvalho.github.io/primeiro-portifolio-responsivo/',
        alt:'Segunda imagem conceitual representando o projeto Primeiro Portfólio',
        icons: ['fab fa-js-square fa-3x', 'fab fa-html5 fa-3x','fab fa-css3-alt fa-3x']

    },
    {
        nome: 'Jogo da Memória',
        img: 'img/projects08.png',
        text: 'Projeto do jogo da memória(Memory Game). Usando HTML, CSS e Javascript Vanilla. O projeto utiliza arrays, objetos, funções, etc. Como no jogo clássico, basta clicar em dois cards e haverá uma verificação, indicando acerto ou erro.',
        link:'https://diogolcarvalho.github.io/projeto-jogo-da-memoria/',
        alt:'Terceira imagem conceitual representando o projeto Jogo da Memória',
        icons: ['fab fa-js-square fa-3x', 'fab fa-html5 fa-3x','fab fa-css3-alt fa-3x']

    },
    {
        nome: 'Lista de Tarefa',
        img: 'img/projects04.jpg',
        text: 'Projeto Lista de Tarefas ou To Do List. Desenvolvido com JavaScript, HTML e CSS. O projeto utiliza Local Storage que armazena os dados no navegador do usuário, para predefinir o tema do projeto. O projeto pode ajudar na organização, em lembretes e tarefas.',
        link:'https://diogolcarvalho.github.io/projeto-To-Do-List/',
        alt:'Quarta imagem conceitual representando o projeto Lista de Tarefa',
        icons: ['fab fa-js-square fa-3x', 'fab fa-html5 fa-3x','fab fa-css3-alt fa-3x']

    },
    {
        nome: 'Fã Page',
        img: 'img/projects03.jpg',
        text: 'Projeto inspirado na tarefa do site <a href="https://www.freecodecamp.org/diogo_lima_carvalho" target="_blank">@freecodecamp</a> que estou cursando. É basicamente uma página de divulgação da cantora, com algumas informações, suas redes e álbuns. Um one page site utilizando HTML, CSS e Javascript para o desenvolvimento projeto. ',
        link:'https://codepen.io/diogolcarvalho/pen/GRmxpmx',
        alt:'Quinta imagem conceitual representando o projeto Fã Page',
        icons: ['fab fa-js-square fa-3x', 'fab fa-html5 fa-3x','fab fa-css3-alt fa-3x']

    }, 
    {
        nome: 'E-commerce',
        img: 'img/projects07.png',
        text: 'Projeto e-commerce. Aqui o foco é criar um site de uma loja de produtos de roupas e sapatos; É possível clicar em um dos produtos e ter mais detalhes, além de uma página de erro se o produto ou link não for encontrado.',
        link:'https://diogolcarvalho.github.io/projeto-eCommerce/',
        alt:'Sexta imagem conceitual representando o projeto E-commerce',
        icons: ['fab fa-js-square fa-3x', 'fab fa-html5 fa-3x','fab fa-css3-alt fa-3x']

    },
    {
        nome: 'Converter Texto',
        img: 'img/projects06.png',
        text: 'Projeto Javascript Vanilla, HTML, CSS, para converter textos. Ao escrever um texto na área selecionada, o usuário pode clicar em um dos botões para transformar em maiúscula, minúscula, texto capitalizado, copiar ou limpar texto.',
        link:'https://diogolcarvalho.github.io/projeto-Converter-Texto/',
        alt:'Sétima imagem conceitual representando o projeto Converter Texto',
        icons: ['fab fa-js-square fa-3x', 'fab fa-html5 fa-3x','fab fa-css3-alt fa-3x']

    },
    {
        nome: 'Validação de Formulários',
        img: 'img/projects05.jpg',
        text: 'Projeto para validar os dados que o usuário digitou no formulário, indicando erro ou acerto de acordo com o digitado. Além de ter campos obrigatórios, que se não forem digitados não serão aceitos corretamente.',
        link:'https://diogolcarvalho.github.io/projeto-validacao-de-formulario/',
        alt:'Oitava imagem conceitual representando o projeto Validação de Formulários',
        icons: ['fab fa-js-square fa-3x', 'fab fa-html5 fa-3x','fab fa-css3-alt fa-3x']
    },
    {
        nome: 'Timer em Javascript',
        img: 'img/projects09.png',
        text: 'A ideia geral é mexer com um timer. A página inicial vai ter uma barra lateral que vai ter todas as opções que o projeto oferece: um timer personalizado, quanto tempo falta para acabar o ano, semana, dia, mês, etc.',
        link:'https://diogolcarvalho.github.io/projeto-timer/',
        alt:'Nona imagem conceitual representando o projeto Timer em Javascript',
        icons: ['fab fa-js-square fa-3x', 'fab fa-html5 fa-3x','fab fa-css3-alt fa-3x']

    },
    {
        nome: 'Jogo da Marvel',
        img: 'img/projects10.png',
        text: 'Como um grande fã da Marvel e um apaixonado por tecnologia e jogos, este projeto sem dúvidas é um dos meus favoritos até agora. O projeto tenta emular os jogos antigos de luta, com os personagens da Marvel.',
        link:'https://marvelgame.netlify.app/',
        alt:'Décima imagem conceitual representando o projeto Jogo da Marvel',
        icons: ['fab fa-js-square fa-3x', 'fab fa-html5 fa-3x','fab fa-css3-alt fa-3x','fab fa-brands fa-bootstrap fa-3x bootstrap', 'bx bxs-data fa-3x api']

    },
    {
        nome: 'Fatec Zona Leste Site',
        img: 'img/projects11.png',
        text: 'Projeto desenvolvido no curso de desenvolvimento web 1° semestre na Fatec Zona Leste. O objetivo era entender os conceitos das aulas e aplicá-los num modelo de site da Fatec, com opções de acessibilidade, responsividade, etc.',
        link:'https://diogolcarvalho.github.io/projeto-site-fatec/',
        alt:'Décima primeira imagem conceitual representando o projeto Fatec Zona Leste Site',
        icons: ['fab fa-js-square fa-3x', 'fab fa-html5 fa-3x','fab fa-css3-alt fa-3x','bx bxl-jquery fa-3x jquery']
    }

]

export default { 
    cardContent: skillCard,
};